...
const ProductDetails = () => {
  ...
  const [selectedSize, setSelectedSize] = useState(null); // chosen size
        ...
        <div className="flex flex-col lg:flex-row gap-10 mt-10">
          <div className="flex flex-col items-center lg:items-start gap-4">
            ...
            <div className="flex gap-3 mt-2">
              {[image, image1, image2, image3].filter(Boolean).map((img, index) => (
                <img
                  key={index}
                  src={img}
                  alt={`thumb-${index}`}
                  ...
                  {sizes.map((s, index) => (
                    <button
                      ...
                      className={`border px-4 py-2 rounded-lg transition ${
                        selectedSize === s ? "bg-blue-500 text-white border-blue-600" : "border-gray-400 text-gray-600 hover:bg-gray-100"
                      }`}
                    >
                      {s}
                    </button>
